...
<head>
    ...
    {{-- Fonts: Cinzel (headings), Inter (body), Press Start 2P (pixel/retro) --}}
...
@php
    ...
    $livesMax = (int) (\App\Models\SystemConfig::get('lives_max') ?? 3);

    $navItems = [
        ['route' => 'student.dashboard', 'label' => 'Dashboard', 'icon' => 'home'],
        ['route' => 'student.missions',  'label' => 'Misi',      'icon' => 'clipboard'],
        ['route' => 'student.swap',      'label' => 'Tukar',     'icon' => 'swap'],
        ['route' => 'leaderboard',       'label' => 'Peringkat', 'icon' => 'trophy'],
        ['route' => 'student.badges',    'label' => 'Badge',     'icon' => 'badge'],
        ['route' => 'student.profile',   'label' => 'Profil',    'icon' => 'user'],
    ];
@endphp
...
<aside class="hidden lg:fixed lg:inset-y-0 lg:left-0 lg:w-60 lg:flex lg:flex-col bg-[#14102a] border-r border-[#2d2050] z-50">
    ...
    <div class="relative" x-data="{ open: false }">
        <button @click="open = !open" class="w-full p-4 border-t border-[#2d2050] flex items-center gap-3 hover:bg-[#2d2050]/30 transition-colors focus:outline-none text-left">
            <img src="{{ $user?->avatar_url }}" alt="Avatar" class="w-9 h-9 rounded-none object-cover pixelated border-2 border-[#f5c518] shrink-0">
            <div class="min-w-0 flex-1">
                <p class="text-sm font-semibold truncate">{{ $user?->name }}</p>
                @if($user?->username)
                    <p class="font-pixel text-[8px] text-[#f5c518] truncate">@<span>{{ $user->username }}</span></p>
                @endif
                <span class="font-pixel text-[8px] text-[#2dc653]">SISWA</span>
            </div>
        </button>
...
<div class="lg:ml-60 flex flex-col min-h-screen">
    ...
    <header class="sticky top-0 z-40 bg-[#14102a] border-b border-[#2d2050] px-4 py-3">
        <div class="flex items-center justify-between">
            <h1 class="font-pixel text-xs text-[#f5c518]">{{ $title ?? 'Piket RPG' }}</h1>
            <div class="flex items-center gap-3">
                ...
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open"
                        ...
                        <img src="{{ $user?->avatar_url }}" alt="Avatar" class="w-full h-full object-cover pixelated border-2 border-[#f5c518]">
                    </button>
    ...
    {{-- STATUS BAR (siswa only) --}}
    @if ($user && $user->role === UserRole::Siswa && $profile)
        <div class="bg-[#14102a] border-b border-[#2d2050] px-3 sm:px-4 py-2 space-y-2 overflow-hidden">
            <x-ui.xp-bar :xp="$profile->xp" />
            <div class="flex flex-wrap items-center justify-between gap-2">
                <x-ui.lives-display :lives="$profile->lives" :max="$livesMax" />
...
<div x-show="mobileMenuOpen" class="fixed inset-0 z-[60] lg:hidden" x-cloak>
    ...
    <div x-show="mobileMenuOpen"
         ...
         <div class="p-4 border-b border-[#2d2050] flex justify-between items-center bg-[#0c0918]">
             <span class="font-pixel text-[10px] text-[#f5c518]">MENU SISWA</span>
